...
publishers_ip_port = sys.argv[1] if len(sys.argv) > 1 else "localhost:5555"
print("publisher ip: {}".format(publishers_ip_port))

publishers = publishers_ip_port.split(',')
subscribed_topics = []

#any other arguments from 2 onwards are the topic filters
if len(sys.argv) > 2:
...
def logger_function(message):
	...
	print('topic_data: {}, message_id: {}, message_sent_at_timestamp: {}, publisher_ip: {}'.format(topic_data, message_id, datetime_sent_at, publisher_ip))
...
sub_middleware = middleware.SubscriberMiddleware(publishers)
